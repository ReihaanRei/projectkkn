                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            ...
                            <div>
                                ...
                                <select id="kelas" name="kelas" required class="select select-bordered w-full">
                                    ...
                                    @foreach (['X IPA 1', 'X IPA 2', 'XI IPA 1', 'XI IPA 2', 'XII IPA 1', 'XII IPA 2'] as $kelas)
                                ...
                                <div class="flex items-center space-x-4">
                                    <div class="avatar m-4">
                                        <div class="w-24 rounded-full ring ring-primary ring-offset-base-100 ring-offset-2">
                                            <img id="preview-foto" src="{{ $siswa->foto ? asset('storage/' . $siswa->foto) : 'https://ui-avatars.com/api/?name=' . urlencode($siswa->nama) }}"
                                                alt="Foto Siswa" />
                                        </div>
                                    </div>
                                    <div>
                                        <label for="foto" class="btn btn-sm btn-outline cursor-pointer">Upload Foto</label>
                                        <input type="file" id="foto" name="foto" accept="image/*" class="hidden" />
                                        <p class="text-xs mt-1">Max size 2MB</p>
                                        @if($siswa->foto)
                                            <p class="text-xs mt-1 text-gray-500">Foto saat ini: {{ basename($siswa->foto) }}</p>
                                        @endif
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-1 gap-6 mb-8">
                            ...
                            <div>
                                ...
                                <input type="text" id="prestasi" name="prestasi"
                                    value="{{ $siswa->prestasi }}" required class="input input-bordered w-full" />
                            </div>
                            ...
                            <div>
                                ...
                                <select id="tingkat" name="tingkat" required class="select select-bordered w-full">
                                    ...
                                    @foreach (['Sekolah', 'Kecamatan', 'Kabupaten/Kota', 'Provinsi', 'Nasional', 'Internasional'] as $tingkat)
                                        <option value="{{ $tingkat }}" {{ $siswa->tingkat == $tingkat ? 'selected' : '' }}>{{ $tingkat }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" id="tahun" name="tahun" min="2000" max="2099"
                                    step="1" value="{{ $siswa->tahun }}" required class="input input-bordered w-full" />
                            </div>
                            ...
                            <div>
                                ...
                                <select id="pencapaian" name="pencapaian" required
                                    ...
                                    @foreach (['Juara 1', 'Juara 2', 'Juara 3', 'Harapan 1', 'Harapan 2'] as $juara)
                                        <option value="{{ $juara }}" {{ $siswa->pencapaian == $juara ? 'selected' : '' }}>{{ $juara }}</option>
                                    @endforeach
                                </select>
                            </div>
